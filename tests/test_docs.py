...
To create a DNS Request Packet:
    ...
    >>> d = Message(header=Header(msg_id=0))
    >>> d.add_question(Question('google.com'))
    >>> print d
    ...
    >>> d = Message(header=Header(msg_id=0))
    >>> d.add_question(Question('google.com', QTYPE.MX))
    >>> print d
    <DNS Header: id=0x0 type=QUERY opcode=QUERY flags=RD rcode=None q=1 a=0 ns=0 ar=0>
...
To create a DNS Response Packet:
    ...
    >>> d = Message(Header(msg_id=0, qr=1, aa=1, rd=1, ra=1))
    >>> d.add_question(Question('abc.com'))
    >>> d.add_answer(RR('abc.com', rtype=QTYPE.A, rdata=A('1.2.3.4')))
    >>> print d
...
To create a skeleton reply to a DNS query:
    ...
    >>> q = Message(header=Header(msg_id=0))
    >>> q.add_question(Question('abc.com', QTYPE.CNAME))
    >>> a = reply(q, data='xxx.abc.com')
    >>> print a
    <DNS Header: id=0x0 type=RESPONSE opcode=QUERY flags=AA,RD,RA rcode=None q=1 a=1 ns=0 ar=0>
